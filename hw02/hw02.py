...
def des(encrypt_or_decrypt, input_file, output_file, key ): 
    ...
    ## write code to carry out 16 rounds of processing
        ...
        if(encrypt_or_decrypt == 'encrypt'):
            xor_result = RE_new ^ round_key[i]
        elif(encrypt_or_decrypt == 'decrypt'):
            xor_result = RE_new ^ round_key[15 - i]
    ...
    #     round_key_new = [0] * 16
    #     for i in range(16):
    #         key,round_key_new[i] = extract_round_key(key,i)
    #         #print "roundkey[{}]".format(i),round_key_new[i]
    #     for i in range(16):
    #         [LE, RE] = bitvec.divide_into_two()
    #         RE_new = RE.permute(expansion_permutation)

    #         xor_result = RE_new ^ round_key_new[15 - i]
    #         RE_new2 = BitVector(size = 0)
    #         for j in range(8):
    #             #row_index = xor_result[j * 6] + xor_result[j * 6 + 5]
    #             row_index = BitVector(size = 2)
    ...
    #     print bitvec.get_text_from_bitvector()
    #     fo = open(output_file,"wb")
    #     fo.writelines(cipher_text)  #write the cipher text to the output file
    #     fo.close()
...
output = des(choice,'output.txt','output2.txt',key)
